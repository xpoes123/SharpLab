"""Minesweeper Race cog — web-based multiplayer game.

Creates a game room on the web server, players join via Discord and play
in their browser. Race to clear a 9x9 minesweeper board first.
"""
    ...
    async def minesweeper(self, interaction: discord.Interaction) -> None:
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.post(
                f"{WEB_API_BASE}/api/v1/minesweeper/rooms",
                json={
                    "host_discord_id": uid,
        ...
        embed = discord.Embed(
            title="\U0001f4a3 Minesweeper Race",
            description=(
        ...
        view = WebMinesweeperLobbyView(room_id, self.bot)
            ...
            try:
                async with httpx.AsyncClient(timeout=5) as client:
                    resp = await client.get(
                        f"{WEB_API_BASE}/api/v1/minesweeper/rooms/{room_id}/result",
                    )
                if resp.status_code != 200:
                    continue
                ...
                embed = discord.Embed(
                    title="\U0001f4a3 Minesweeper Race \u2014 Results",
...
class WebMinesweeperLobbyView(ui.View):
    ...
    async def join_btn(
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.post(
                    f"{WEB_API_BASE}/api/v1/minesweeper/rooms/{self.room_id}/tokens",
                    json={
                        "discord_user": uid,
