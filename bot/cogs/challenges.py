...
class ChallengesCog(commands.Cog):
    ...
    @app_commands.command(name="daily", description="View today's daily challenges")
    async def daily(self, interaction: discord.Interaction) -> None:
        ...
        # Re-evaluate incomplete challenges (in case they were completed
        # between background-task ticks)
        newly_completed = 0
        for slot in slots:
            if slot["completed"]:
                continue
        ...
        coins_earned = completed_count * CHALLENGE_COINS
        ...
        # Build embed
        embed = discord.Embed(
            title=f"Daily Challenges \u2014 {_format_date_display(today)}",
            color=0xF1C40F,
        )
        ...
        for i, slot in enumerate(slots, start=1):
            template = _get_template(slot["challenge_id"])
            desc = template.description if template else slot["challenge_id"]
            if slot["completed"]:
                lines.append(f"{i}. \u2705 {desc} \u2014 +{CHALLENGE_COINS}c")
            else:
                lines.append(f"{i}. \u2b1c {desc}")

        lines.append("")
        lines.append(f"Completed: **{completed_count}/3**")

        if completed_count < 3:
            lines.append(f"Bonus: Complete all 3 for +{BONUS_COINS}c")
        elif bonus_claimed:
            lines.append(f"\u2705 All 3 done! **+{BONUS_COINS}c bonus** claimed!")
        else:
            lines.append(f"\u2705 All 3 done! Bonus already claimed.")

        embed.description = "\n".join(lines)

        if coins_earned > 0:
            embed.set_footer(text=f"Earned today: {coins_earned}c from challenges")

        # Streak info
        streak = await queries.get_consecutive_daily_completions(uid, today)
        if completed_count == 3:
            streak += 1  # include today
        if streak > 1:
            embed.set_footer(
                text=f"Earned today: {coins_earned}c | Streak: {streak} day{'s' if streak != 1 else ''}"
                if coins_earned > 0
                else f"Streak: {streak} day{'s' if streak != 1 else ''}",
            )
