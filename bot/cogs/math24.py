...
def _betting_embed(table: Math24Table) -> discord.Embed:
    embed = discord.Embed(
        title="Math 24 \u2014 Race",
        description=(
            f"Race to solve Math 24 puzzles! **First to {WINS_TO_WIN} wins**.\n"
...
def _final_embed(table: Math24Table) -> discord.Embed:
    max_wins = max((p.rounds_won for p in table.players.values()), default=0)
...
class Math24TableView(ui.View):
    ...
    async def _end_game(self) -> None:
        """End the race and show final results."""
        table = self.table
        table.phase = "closed"
        ...
        if len(table.players) >= 2:
            ...
            try:
                await update_elo_multiplayer(finish_order, "math24", "math24")
            except Exception:
                pass
