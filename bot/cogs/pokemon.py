"""Pokemon cog — multiplayer /pokemon guessing game.

Progressive hints about a Pokemon; first to type its name wins the round.
First to WINS_TO_WIN rounds wins. ELO-ranked.
"""
...
def _betting_embed(table: PokeTable) -> discord.Embed:
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"**First to {WINS_TO_WIN} wins** takes the title.\n"
...
def _final_embed(table: PokeTable) -> discord.Embed:
    ...
    for i, p in enumerate(sorted_players):
        ...
        lines.append(f"{medal} **{p.display_name}** \u2014 {p.rounds_won} rounds won")
...
class PokeTableView(ui.View):
    ...
    @ui.button(
        label="Rejoin", style=discord.ButtonStyle.primary, emoji="\U0001f504", row=0,
    )
    async def rebet_btn(
        self, interaction: discord.Interaction, button: ui.Button,
    ) -> None:
