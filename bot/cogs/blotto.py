"""Colonel Blotto cog — web-based multiplayer strategy game.

Creates a game room on the web server, players join via Discord and play
in their browser. Allocate 100 soldiers across 5 battlefields.
"""
    ...
    async def blotto(self, interaction: discord.Interaction) -> None:
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.post(
                f"{WEB_API_BASE}/api/v1/blotto/rooms",
                json={
                    "host_discord_id": uid,
        ...
        embed = discord.Embed(
            title="\u2694\ufe0f Colonel Blotto",
            description=(
                "Deploy **100 soldiers** across **5 battlefields**.\n"
                "Win the most battlefields to take the round.\n"
                "**First to 3 round wins** takes the match!\n\n"
                "Click **Join** below to get your game link."
            ),
            colour=discord.Colour.dark_gold(),
        )
        embed.set_footer(text=f"Room {room_id} \u2022 First to 3 wins \u2022 2-8 players")

        view = WebBlottoLobbyView(room_id, self.bot)
            ...
            try:
                async with httpx.AsyncClient(timeout=5) as client:
                    resp = await client.get(
                        f"{WEB_API_BASE}/api/v1/blotto/rooms/{room_id}/result",
                    )
                if resp.status_code != 200:
                    continue
                ...
                embed = discord.Embed(
                    title="\u2694\ufe0f Colonel Blotto \u2014 Results",
                ...
                for i, r in enumerate(result.get("results", [])):
                    ...
                    lines.append(
                        ...
                        f"{r['rounds_won']}W / {r['battlefields_won_total']} BFs"
                    )
...
# ── Web Blotto Lobby View ────────────────────────────────────────────────


class WebBlottoLobbyView(ui.View):
    ...
    async def join_btn(
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.post(
                    f"{WEB_API_BASE}/api/v1/blotto/rooms/{self.room_id}/tokens",
                    json={
                        "discord_user": uid,
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(BlottoCog(bot))
