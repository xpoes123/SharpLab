...
Auto-maintained by Sentinel's memory system. Last updated: 2026-04-22 22:23 UTC
...
- Temporal workflows should guard against replay of partial side effects (confidence: 40)
- Validate line data completeness before processing (confidence: 37)
- Test with realistic data volumes; unit tests may miss batch edge cases (confidence: 31)
...
- Avoid assuming code is broken if infrastructure exists (confidence: 26)

## Conventions & Preferences

- Document intentional trade-offs when reordering state updates and side effects (confidence: 34)
...
- Prioritize silent failures over duplicate notifications in bot integrations (confidence: 32)
- Document why fallback behavior is acceptable in code comments (confidence: 31)
...
- Async crash windows between message send and state persistence (confidence: 37)
- Empty sequence check before max()/min() on generators (confidence: 36)
- DB state updates after external API calls should be atomic or pre-committed (confidence: 32)
- Test with boundary cases (24, 25, 26, 50 items) before deployment (confidence: 31)
