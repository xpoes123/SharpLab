...
## 23. Hex (Web)
Two players on a rhombus-shaped board of hexagons (11x11 standard). One connects top-to-bottom, the other left-to-right. Place one stone per turn. No draws are possible (Brouwer fixed-point theorem guarantees exactly one winner). First player has a proven advantage, balanced by the **swap rule** — after the first move, the second player can choose to swap sides. Zero luck, infinite depth, simple rules. The "Go" of connection games. Web UI: clickable hex grid, color-coded paths, highlight winning chain on game end. Perfect for ELO — pure skill expression. 1v1 only.

## 24. Set (Web)
Cards with 4 attributes: shape (diamond/oval/squiggle), color (red/green/purple), count (1/2/3), fill (solid/striped/empty). 12 cards dealt face-up. Players race to spot valid **sets** — 3 cards where each attribute is either all the same or all different across the trio. First to call a valid set scores a point; cards are replaced. Invalid calls lose a point. Game ends when the deck is exhausted and no sets remain. Pure pattern recognition at speed. Web UI: card grid with click-to-select (highlight 3, auto-validate), live scoreboard, timer. 2-6 players. Rounds are ~10 min.

## 25. Codenames (Web)
5x5 grid of words. Two teams, each with a **spymaster** who sees which words belong to their team (color-coded key). Spymasters take turns giving a one-word clue + a number ("animal 3" = 3 of our words relate to "animal"). Their team discusses and taps words one at a time. Correct = keep going (up to the number + 1). Hit a neutral = turn over. Hit the opponent's word = turn over + helps them. Hit the **assassin** = instant loss. First team to find all their words wins. Web UI: word grid (spymaster sees colors, guessers see blank), clue input, turn indicator. 4-8 players (2 per team minimum). The best team word game — drives massive engagement and replayability.

## 26. Spot It (Web)
Deck of cards, each with 8 symbols. Any two cards always share **exactly one** symbol in common (projective plane geometry). Two cards flipped simultaneously — first player to spot and call the matching symbol wins the pair. Most pairs at the end wins. Pure visual processing speed + reaction time. Web UI: two large cards displayed, players click the matching symbol on either card. Symbols randomized in size/rotation to prevent memorization. 2-6 players. Rounds are fast (~3 min per deck). Variant: "hot potato" mode — you must find the match to pass the card, last holding it loses a point.

## 27. Windmill (Web, Daily)
IMO-2025-P6-flavored tile-placement puzzle. Each day the player gets a shuffled bag of rectangular tiles and an n×n grid; place every tile (rotations allowed, no overlaps, all in-bounds) so that **exactly one unit square in each row and each column is left uncovered.** Offline/client-side solve, server just validates the final placement — no new DB tables or endpoints needed, same shape as the trap-the-pig / rush-hour daily plugins. Ranks by time (fastest valid solve), since tile count is fixed per day and can't be gamed. Difficulty scales the grid: easy 6×6 (~8-12 tiles), medium 8×8 (~12-18), hard 10×10 (~16-24). Would be the platform's 4th daily game. First-run gets a short interactive tutorial on a fixed 4×4 mini board. Design already written up: `docs/superpowers/specs/2026-09-18-windmill-daily-game-design.md` (currently only on `origin/autosave-backup` — no implementation yet, no branch pushed under its own name).
