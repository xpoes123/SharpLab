...
async def public_create_room(body: PublicJoinRequest):
    ...
    room_id = secrets.token_hex(4)
    room = MinesweeperRoom(room_id=room_id, host_id=user_id, channel_id="web")
    rooms[room_id] = room
    await queries.create_game_session(room_id, "minesweeper", user_id, "web")
    token = secrets.token_hex(16)
    ...
    base = os.environ.get("WEB_BASE_URL", "https://sharplab.djiang.xyz")
    return {"room_id": room_id, "token": token, "url": f"{base}/minesweeper/{room_id}?t={token}"}
...
async def public_join_room(room_id: str, body: PublicJoinRequest):
    ...
    await _broadcast_room_state(room)
    base = os.environ.get("WEB_BASE_URL", "https://sharplab.djiang.xyz")
    return {"token": token, "url": f"{base}/minesweeper/{room_id}?t={token}"}
...
async def cleanup_stale_minesweeper_rooms() -> None:
    ...
    while True:
        ...
        try:
            ...
            logger.exception("cleanup_stale_minesweeper_rooms error — loop continues")
