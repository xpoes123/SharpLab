// SharpLab HQ — Soccer Sim. Bet the two-way moneyline of a simulated soccer
// match for coins. Ties are broken by extra time + penalties in the sim, so
// there's no draw — every match has a winner. /new signs the matchup + true home
// win prob into a token; /bet decodes it, debits the stake, draws the winner
// (CSPRNG weighted on the true prob so the priced payout is exact), pays out, and
// returns the authoritative new balance which we push to the nav chip + on-page
// header. Soccer flavor: goals, halves.
...
// ── POST to a soccersim endpoint. Returns parsed JSON or {error}. ──
async function postSim(path, body) {
  if (MOCK) return mockSim(path, body);
  const r = await fetch("/api/v1/casino/soccersim" + path, {
    method: "POST",
    credentials: "include",
...
async function placeBet() {
  ...
  $("simBanner").innerHTML = `<span class="simdots">Kick off${REDUCE ? "" : "<i>.</i><i>.</i><i>.</i>"}</span>`;
...
function renderResult(res) {
  ...
  $("simArena").insertAdjacentHTML("beforeend", `
    ...
    <div class="fruns">Full time · goals</div>`);
...
function buildPage() {
  ...
  app.innerHTML = `
    <div class="sim-head">
      <h1>⚽ Soccer Sim <span class="balancechip" id="pageBal">${coins(state.balance)}</span></h1>
      <p>Pick a moneyline, then sim the match. Ties break on penalties — someone always wins. Paid at the true-probability price.</p>
    </div>
    ${signedOut}
...
const MOCK_TEAMS = [
  ["RMA", "Real Madrid"], ["BAR", "Barcelona"], ["MCI", "Man City"], ["ARS", "Arsenal"],
  ["LIV", "Liverpool"], ["BAY", "Bayern Munich"], ["PSG", "PSG"], ["INT", "Inter Milan"],
];
...
function mockSim(path, body) {
  ...
  let hs = 0, as = 0;
  for (let i = 0; i < 2; i++) {
    if (Math.random() < g.homeProb * 0.65) hs += 1;
    if (Math.random() < (1 - g.homeProb) * 0.65) as += 1;
  }
  if (winner === "home" && hs <= as) hs = as + 1 + Math.floor(Math.random() * 2);
  if (winner === "away" && as <= hs) as = hs + 1 + Math.floor(Math.random() * 2);
