// SharpLab HQ — Craps (pass line). Roll the dice for coins.
// Come-out roll: 7/11 wins (2×), 2/3/12 craps out (lose). Anything else sets a
// POINT — keep rolling until the point repeats (win 2×) or a 7 shows (seven out,
// lose). Rounds POST to /api/v1/casino/craps/* (session-cookie auth); each
// resolved response carries the authoritative new balance, which we push into
...
// ── POST to a craps endpoint. Returns parsed JSON or {error}. ──
async function postCraps(path, body) {
  if (MOCK) return mockCraps(path, body);
  const r = await fetch("/api/v1/casino/craps" + path, {
    method: "POST",
    credentials: "include",
...
function buildPage() {
  ...
  app.innerHTML = `
    <div class="cr-head">
      <h1>🎲 Craps <span class="balancechip" id="pageBal">${coins(state.balance)}</span></h1>
      <p>Pass line. 7 or 11 wins the come-out; 2/3/12 craps out. Otherwise a point is set — roll it again before a 7. Pays 2×.</p>
    </div>
    ${signedOut}
...
function mockCraps(path, body) {
  if (path === "/comeout") {
    ...
    if (total === 2 || total === 3 || total === 12) {
      ...
      return { done: true, dice, total, result: "craps", payout: 0, balance: state.balance };
    }
