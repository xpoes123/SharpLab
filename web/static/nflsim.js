// SharpLab HQ — NFL Sim. Bet the moneyline of a simulated NFL game for coins.
...
// ── POST to an nflsim endpoint. Returns parsed JSON or {error}. ──
async function postSim(path, body) {
  if (MOCK) return mockSim(path, body);
  const r = await fetch("/api/v1/casino/nflsim" + path, {
    method: "POST",
    credentials: "include",
...
function buildPage() {
  ...
  app.innerHTML = `
    <div class="sim-head">
      <h1>🏈 NFL Sim <span class="balancechip" id="pageBal">${coins(state.balance)}</span></h1>
...
const MOCK_TEAMS = [
  ["KC", "Chiefs"], ["BAL", "Ravens"], ["PHI", "Eagles"], ["BUF", "Bills"],
  ["SF", "49ers"], ["DET", "Lions"], ["DAL", "Cowboys"], ["GB", "Packers"],
];
...
function mockSim(path, body) {
  if (path === "/new") {
    ...
    const spread = (Math.round((homeProb - 0.5) * 56) || 0) + (homeProb >= 0.5 ? 0.5 : -0.5);
    const total = Math.round((40 + Math.random() * 12) * 2) / 2; // 40–52
  ...
  let hs = Math.max(3, Math.round(half + g.spread / 2 + (Math.floor(Math.random() * 15) - 7)));
  let as = Math.max(3, Math.round(half - g.spread / 2 + (Math.floor(Math.random() * 15) - 7)));
  if (winner === "home" && hs <= as) hs = as + 3 + Math.floor(Math.random() * 7);
  if (winner === "away" && as <= hs) as = hs + 3 + Math.floor(Math.random() * 7);
