...
async def public_create_room(body: PublicJoinRequest):
    ...
    room_id = secrets.token_hex(4)
    room = FiggieRoom(room_id=room_id, host_id=user_id, channel_id="web")
    rooms[room_id] = room
    await queries.create_game_session(room_id, "figgie", user_id, "web")
    token = secrets.token_hex(16)
    room.players[user_id] = WebFiggiePlayer(
    ...
    base = os.environ.get("WEB_BASE_URL", "https://sharplab.djiang.xyz")
    return {"room_id": room_id, "token": token, "url": f"{base}/figgie/{room_id}?t={token}"}
