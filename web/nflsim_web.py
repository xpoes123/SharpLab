"""Web NFL Sim — solo moneyline bet on a simulated NFL game. Two stateless steps:
...
Reuses the Discord cog's pure sim math (bot/cogs/nflsim.py) so the two stay in sync.
"""
...
def _final_score(winner: str, home_prob: float, total: float, spread: float) -> tuple[int, int]:
    """A plausible football final consistent with the drawn winner — flavor only,
    never changes who won. Base each team around total/2 (NFL games run low, ~44),
    tilt by the spread, add football-scale noise, then force the winner strictly
    ahead by a field-goal-ish margin."""
    half = total / 2
    # spread is home's margin (positive = home favored); split it across the two.
    home = half + spread / 2 + (secrets.randbelow(15) - 7)
    away = half - spread / 2 + (secrets.randbelow(15) - 7)
    home, away = max(3, round(home)), max(3, round(away))
    if winner == "home" and home <= away:
        home = away + 3 + secrets.randbelow(7)
    elif winner == "away" and away <= home:
        away = home + 3 + secrets.randbelow(7)
    return home, away
...
async def place_bet(request: Request, body: BetBody):
    ...
    await queries.log_casino_result(uid, "nflsim", stake, payout)
