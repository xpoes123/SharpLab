"""Web MLB Sim settlement — money paths. Forces a deterministic winner by
...
def _force_winner(monkeypatch, side):
    ...
    if side == "home":
        monkeypatch.setattr(mlbsim_web.secrets, "randbelow", lambda n: 0)
    else:
        monkeypatch.setattr(mlbsim_web.secrets, "randbelow", lambda n: n - 1)
...
def test_winning_bet_credits_stake_times_mult_and_logs(monkeypatch, tmp_path):
    ...
    async def go():
        await sch.init_db()
        monkeypatch.setattr(mlbsim_web.auth, "read_session", lambda r: {"id": "u1"})
        ...
        new = await mlbsim_web.new_game(_Req())
        ...
        stake = 500
        res = await mlbsim_web.place_bet(
            _Req(), mlbsim_web.BetBody(token=token, side="home", stake=stake),
        )
        ...
        # Logged exactly one mlbsim round.
...
def test_losing_bet_pays_zero(monkeypatch, tmp_path):
    ...
    async def go():
        await sch.init_db()
        monkeypatch.setattr(mlbsim_web.auth, "read_session", lambda r: {"id": "u2"})
        ...
        stake = 750
        res = await mlbsim_web.place_bet(
            _Req(), mlbsim_web.BetBody(token=new["token"], side="home", stake=stake),
        )
...
def test_payout_recomputed_server_side_and_forged_token_rejected(monkeypatch, tmp_path):
    ...
    async def go():
        await sch.init_db()
        monkeypatch.setattr(mlbsim_web.auth, "read_session", lambda r: {"id": "u3"})
        ...
        bad = await mlbsim_web.place_bet(
            _Req(), mlbsim_web.BetBody(token=forged, side="home", stake=stake),
        )
...
def test_overdraw_rejected(monkeypatch, tmp_path):
    ...
    async def go():
        await sch.init_db()
        monkeypatch.setattr(mlbsim_web.auth, "read_session", lambda r: {"id": "u4"})
        await queries.get_or_create_casino_wallet("u4")
        bal = await queries.get_casino_balance("u4")

        new = await mlbsim_web.new_game(_Req())
        res = await mlbsim_web.place_bet(
            _Req(), mlbsim_web.BetBody(token=new["token"], side="home", stake=bal + 1),
        )
